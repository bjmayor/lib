...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	return [self tableView:tableView IASKCellheightForSpecifier:specifier];
}

- (CGFloat)tableView:(UITableView *)tableView IASKCellheightForSpecifier:(IASKSpecifier*)specifier
{
    CGFloat height = 0;
    if ([specifier.type isEqualToString:kIASKPSToggleSwitchSpecifier])
    {
        height = [specifier.title sizeWithFont:[UIFont boldSystemFontOfSize:[UIFont labelFontSize]] constrainedToSize:CGSizeMake(self.tableView.width - 6 * KGap, CGFLOAT_MAX)].height + 2 * KGap ;
        
    }
    else if([specifier.type isEqualToString:kIASKPSTitleValueSpecifier] || [specifier.type isEqualToString:kIASKOpenURLSpecifier] || [specifier.type isEqualToString:kIASKButtonSpecifier] || [specifier.type isEqualToString:kIASKMailComposeSpecifier])
    {
        height = [specifier.title sizeWithFont:[UIFont boldSystemFontOfSize:[UIFont labelFontSize]] constrainedToSize:CGSizeMake(self.tableView.width - 2 * KGap, CGFLOAT_MAX)].height + 2 * KGap;
    }
...
- (UITableViewCell*)newCellForIdentifier:(NSString*)identifier {
	...
	if ([identifier isEqualToString:kIASKPSToggleSwitchSpecifier]) {
		...
		cell.accessoryView = [[IASKSwitch alloc] initWithFrame:CGRectMake(0, 0, 55, 31)];
    ...
    UIColor *textColor = [HaloTheme colorWithColorId:@"font_color1"];
    if (textColor)
    {
