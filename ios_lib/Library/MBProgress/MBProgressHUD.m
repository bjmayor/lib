...
+ (MBProgressHUD*)showInfoHUD:(NSString*)text warning:(BOOL)warning showDelay:(NSInteger)showDelay
{
	MBProgressHUD *infoHUD = [MBProgressHUD showHUDAddedTo:APPWindow animated:YES];
	infoHUD.customView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:warning?@"Halo.bundle/images/info_warning":@"Halo.bundle/images/info_done"]];
    infoHUD.mode = MBProgressHUDModeCustomView;
	infoHUD.detailsLabelText = text;
    infoHUD.detailsLabelFont = [UIFont boldSystemFontOfSize:16.f];
	infoHUD.minShowTime = showDelay;
	[infoHUD hide:YES];
    return infoHUD;
}

+ (MBProgressHUD*)showInfoHUD:(NSString*)text view:(UIView*)view warning:(BOOL)warning showDelay:(NSInteger)showDelay frame:(CGRect)frame
{
	MBProgressHUD *infoHUD = [[MBProgressHUD alloc] initWithView:view];
    infoHUD.frame = frame;
    [view addSubview:infoHUD];
    [infoHUD show:YES];
...
- (void)layoutSubviews {
	...
	if (parent)
    {
        ...
        //this is for the keyboard maybe shown
        if (self.frame.size.height > CGRectGetHeight(myRect))
        {
            self.frame = CGRectMake(self.frame.origin.x, self.frame.origin.y
                                    , self.frame.size.width, CGRectGetHeight(myRect));
        }
	}
