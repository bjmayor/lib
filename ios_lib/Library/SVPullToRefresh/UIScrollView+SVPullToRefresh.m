...
- (void)refreshDone
{
    ...
    [self.pullToRefreshView setSubtitle:[NSString stringWithFormat:NSLocalizedStringFromTableInBundle(@"core.ego_header_update_date",@"Global", [Halo bundle] ,nil), [self.pullToRefreshView.dateFormatter stringFromDate:[NSDate date]]] forState:SVPullToRefreshStateAll];
}


- (BOOL)isRefreshing
{
    BOOL ret = (self.pullToRefreshView.state == SVPullToRefreshStateLoading);
    return ret;
}
...
- (id)initWithFrame:(CGRect)frame {
    if(self = [super initWithFrame:frame]) {
        ...
        self.titles = [NSMutableArray arrayWithObjects:NSLocalizedString(@"core.ego_pulldown_get_refresh", nil),
                       NSLocalizedString(@"core.ego_release_refresh", nil),
                       NSLocalizedString(@"core.ego_status_loading", nil),
                       nil];
...
- (void)scrollViewDidScroll:(CGPoint)contentOffset {
    if (self.scrollView.infiniteScrollingView.state != SVInfiniteScrollingStateStopped && self.scrollView.infiniteScrollingView.state != SVInfiniteScrollingStateNoMore)
    {
        return;
    }
