...
- (BOOL)isLoadingMore;
{
    return self.infiniteScrollingView.state == SVInfiniteScrollingStateLoading;
}

- (void)setNoMoreData
{
    self.infiniteScrollingView.state =  SVInfiniteScrollingStateNoMore;
}

- (BOOL)supportInfinitScroll
{
    return self.infiniteScrollingView.infiniteScrollingHandler != nil;
}
...
- (id)initWithFrame:(CGRect)frame {
    if(self = [super initWithFrame:frame]) {
        ...
        self.viewForState = [NSMutableArray arrayWithObjects:@"", @"", @"", @"", NSLocalizedStringFromTableInBundle(@"core.ego_footer_no_more_data",@"Global", [Halo bundle],nil),nil];
    }
...
- (void)scrollViewDidScroll:(CGPoint)contentOffset
{
    ...
    if(self.state != SVInfiniteScrollingStateLoading && self.state != SVInfiniteScrollingStateNoMore && [self isEnableInfinitScroll] && self.scrollView.pullToRefreshView.state == SVPullToRefreshStateStopped && contentOffset.y > 0)
    {
...
- (void)setState:(SVInfiniteScrollingState)newState {
    ...
    if(_state == newState ||_state == SVInfiniteScrollingStateNoMore)
    {
        return;
    }
    ...
    if(previousState == SVInfiniteScrollingStateTriggered && newState == SVInfiniteScrollingStateLoading && self.infiniteScrollingHandler && [self isEnableInfinitScroll])
    {
...
- (BOOL)isEnableInfinitScroll
{
    if(self.enableBlock)
    {
        return self.enableBlock();
    }
    else
    {
        return YES;
