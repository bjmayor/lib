...
- (void)downloadWithURL:(NSURL *)url delegate:(id<SDWebImageManagerDelegate>)delegate options:(SDWebImageOptions)options userInfo:(NSDictionary *)userInfo
{
    ...
    if (options & (1 << 4))
    {
        cachedImage = [[SDImageCache sharedImageCache] imageFromKey:[self cacheKeyForURL:url]];
    }
    
    if (cachedImage)
    {
        [self imageCache:[SDImageCache sharedImageCache] didFindImage:cachedImage forKey:[self cacheKeyForURL:url] userInfo:info];
    }
    else
    {
...
- (void)imageDownloader:(SDWebImageDownloader *)downloader didFinishWithImage:(UIImage *)image
{
    ...
    if (image)
    {
        if ( !(options & SDWebImageNotCache))
        {            
            if (options & SDWebImageCacheDiscOnly)
            {
                [[SDImageCache sharedImageCache] storeImageOnlyToDisk:image forKey:[self cacheKeyForURL:downloader.url]];
            }
            else
            {
